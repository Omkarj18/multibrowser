...
//			firefoxOptions.addArguments("--disable-dev-shm-usage");
            ...
            //2. Set SSL and HTTP proxy in SeleniumProxy
            Proxy seleniumProxy3=new Proxy();
            seleniumProxy3.setHttpProxy("localhost:" +myProxy.getPort());
            seleniumProxy3.setSslProxy("localhost:" +myProxy.getPort());

            //3. Add Capability for PROXY in DesiredCapabilities
            DesiredCapabilities capability=new DesiredCapabilities();
            capability.setCapability(CapabilityType.PROXY, seleniumProxy3);
            capability.acceptInsecureCerts();
            capability.setCapability(CapabilityType.ACCEPT_INSECURE_CERTS, true);
...
//			//6. HAR name
            ...
            // Configure the desired capabilities to use the proxy
            ...
            options.setCapability("safari:insecure", true);
            ...
            options.setCapability("safari:headless", true);
...
//			File myHARFile=new File(System.getProperty("user.dir")+"/target/reports/HARFolder/Failed"+new_scenario_name+".har");
            File HARFolder=new File(System.getProperty("user.dir")+"/target/reports/HARFolder");
//			har.writeTo(myHARFile);
            if (!HARFolder.exists()) {
                boolean created = HARFolder.mkdirs();
                if (created) {
                    System.out.println("Folder created successfully.");
                    File myHARFile=new File(System.getProperty("user.dir")+"/target/reports/HARFolder/Failed"+scenario_name+".har");
                    har.writeTo(myHARFile);
                } else {
